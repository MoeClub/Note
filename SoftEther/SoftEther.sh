...
while [[ $# -ge 1 ]]; do
  case $1 in
    ...
    -port)
      shift
      SoftEtherPort=`echo "$1" |sed 's/[[:space:]]//g'`
      shift
      ;;
    -url)
      shift
...
[ "$SoftEtherPort" == "5555" ] || sed -i "s/uint Port 5555/uint Port ${SoftEtherPort}" /etc/softether/vpn_server.config
